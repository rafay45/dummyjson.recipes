...
let getRecipe = document.getElementById('recipess');
for (var i = 0; i < recipes.length; i++) {
    getRecipe.innerHTML += `<div onclick="info('${recipes[i].image}')" id="recipe-image"><img src="${recipes[i].image}" alt=""></div> `
}
let images = ["https://cdn.dummyjson.com/recipe-images/1.webp",
...
let chartsGet = document.getElementById('charts')
let innerGet = document.getElementById('inner-chart')
function info(recipesImage) {
    chartsGet.style.display = "flex"
    innerGet.innerHTML = "";
    for (var j = 0; j < images.length; j++) {
        if (recipesImage === images[j]) {
            for (var k = 0; k < recipes.length; k++) {
                if (recipes[k].image === images[j]) {
                    innerGet.innerHTML = `<img src="${images[j]}" alt="">
                  <p><span>PrepTimeMinutes:</span> ${recipes[k].prepTimeMinutes}</p>
                        <p><span>CookTimeMinutes:</span> ${recipes[k].cookTimeMinutes}</p>
                        <p><span>Servings:</span> ${recipes[k].servings}</p>
                        <p><span>Difficulty:</span> ${recipes[k].difficulty}</p>
                        <p><span>Cuisine:</span> ${recipes[k].cuisine}</p>
                        <p><span>CaloriesPerServing:</span> ${recipes[k].caloriesPerServing}</p>
                        <p><span>Tags:</span> ${recipes[k].tags}</p>
                        <p><span>UserId:</span> ${recipes[k].userId}</p>
                        <p><span>Rating:</span> ${recipes[k].rating}</p>
                        <p><span>ReviewCount:</span> ${recipes[k].reviewCount}</p>
                        <p><span>MealType:</span> ${recipes[k].mealType}</p>
                `
                }
            }
        }
